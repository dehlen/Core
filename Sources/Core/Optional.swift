extension Optional {
    ...
    /// I.e let gift = try g.or(throw: ServerError(privateMessage: "No such gift"))
...
extension Optional {
    ...
    /// I.e if user != nil, let account = userAccount() ... if let account = user.and(userAccount())
    ...
    /// I.e let children = database.current().and(then: { database.spouse($0) })
...
extension Optional {
    ...
    /// I.e self.user.on(some: { AppCoordinator.shared.unlock() })
    func on(some: () throws -> Void) rethrows {
        if self != nil { try some() }
    }

    /// Executes the closure `none` if and only if the optional has no value
    /// I.e self.user.on(none: { AppCoordinator.shared.logout() })
...
extension Optional {
    ...
    /// I.e valueLabel.expect("valueLabel not connected in IB").text = state.title
